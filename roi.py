...
def _fetch_ringback_bookings(business_id):
    """GET booked jobs from the connected RingBack instance. Returns a list of booking
    dicts (RingBack's shape). Factored out so the suite can stub the HTTP call. Lazily
    imports requests (only hit when a connection exists), mirroring publishing/messaging.

    The exact bookings GET shape must be matched to RingBack's real API; the parsing in
    sync_ringback reads conservatively (id, channel, value, label) with safe defaults."""
    import requests  # lazy: only when RingBack is actually configured
    resp = requests.get(
        RINGBACK_API_URL.rstrip("/") + "/bookings",
        headers={"Authorization": f"Bearer {RINGBACK_API_KEY}"},
...
def sync_ringback(business_id):
    ...
    Honest modes:
      ...
      - connected     -> actually GET the bookings and add_conversion(... origin='ringback')
    ...
    try:
        bookings = _fetch_ringback_bookings(business_id)
    except Exception as e:
        print(f"[jobmagnet] ringback sync failed for business {business_id}: {e}", flush=True)
    ...
    for b in bookings:
        ...
        ext_id = str(ext_id)
        if db.conversion_exists(business_id, "ringback", ext_id):
        ...
        db.add_conversion(business_id, channel, status="booked", value=value,
                          label=label, origin="ringback", ext_id=ext_id)
        added += 1
