"""Postgres storage for JobMagnet (psycopg3). DATABASE_URL points at the server.
...
import lsa       # pure module -- canonical LSA Concierge checklist keys
import radiusmail  # pure module -- Neighbor Mail statuses
...
def init_db():
    ...
    conn.execute(
        ...
        -- JobMagnet's engine: diagnostic signals (the Walkthrough answers) + the Mandate
        ...
        -- Command-center memory: every conversation the owner has with JobMagnet, so we can
        ...
        -- heartbeat), so the owner can see what JobMagnet did unattended. Phase 5 reads this.
        ...
        -- Neighbor Mail: one radius direct-mail campaign per completed jobsite.
        -- v0 is assisted (print-it-yourself + USPS EDDM); mode flips to live only
        -- when a print/mail provider is wired. See radiusmail.py.
        CREATE TABLE IF NOT EXISTS mail_campaigns (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            business_id INTEGER NOT NULL,
            contact_id INTEGER,                      -- the customer whose job anchors it
            job_address TEXT NOT NULL,               -- jobsite address (PII: tenant's customer)
            service TEXT,
            pieces INTEGER NOT NULL DEFAULT 50,
            letter_body TEXT,
            hanger_body TEXT,
            status TEXT NOT NULL DEFAULT 'draft',    -- draft | approved | printed
            mode TEXT NOT NULL DEFAULT 'assisted',   -- assisted (no mail API wired yet)
            created_at TEXT,
            printed_at TEXT
        );
        CREATE INDEX IF NOT EXISTS idx_mail_biz ON mail_campaigns(business_id, created_at);

        -- LLM cost ledger: one row per real (non-demo) brain.generate() call. Powers the
        -- daily spend caps (platform + per-tenant) and future cost-of-serve reporting.
        -- business_id NULL = a platform-level call not tied to one tenant.
        CREATE TABLE IF NOT EXISTS llm_usage (
            id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            business_id INTEGER,
            tier TEXT,                               -- bulk | brand
            provider TEXT,                           -- claude | minimax | deepseek
            model TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_llm_usage_biz ON llm_usage(business_id, created_at);
        """
    )
    ...
    # New-client engines: jobsite address on a customer (anchors Neighbor Mail; PII —
    # the tenant's own customer data, entered by the tenant) and the partner type that
    # drives the Partner engine's per-type intro + anti-kickback guardrail.
    _ensure_columns(c, "contacts", {"address": "TEXT", "partner_type": "TEXT"})
...
def count_users():
    conn = get_conn()
    n = conn.execute("SELECT COUNT(*) FROM users").fetchone()["count"]
    conn.close()
    return n
...
def create_business(fields):
    ...
    marks = ",".join("%s" for _ in range(len(cols) + 1))
    cur = conn.execute(f"INSERT INTO businesses ({collist}) VALUES ({marks}) RETURNING id",
...
def update_business(business_id, fields):
    ...
    conn.execute(f"UPDATE businesses SET {sets} WHERE id=%s",
...
def get_post(post_id, business_id=None):
    ...
    if business_id is None:
        ...
        row = conn.execute("SELECT * FROM content_posts WHERE id=%s AND business_id=%s",
...
def list_posts(business_id, status=None):
    ...
    if status:
        rows = conn.execute(
            "SELECT * FROM content_posts WHERE business_id=%s AND status=%s "
            "ORDER BY created_at DESC", (business_id, status)).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM content_posts WHERE business_id=%s ORDER BY created_at DESC",
...
def set_post_status(post_id, business_id, status):
    ...
    conn = get_conn()
    cur = conn.execute(
        "UPDATE content_posts SET status=%s, decided_at=%s WHERE id=%s AND business_id=%s",
...
def set_post_published(post_id, business_id, mode):
    ...
    conn = get_conn()
    conn.execute(
        "UPDATE content_posts SET status='published', publish_mode=%s, decided_at=%s "
...
def update_post_body(post_id, business_id, body):
    conn = get_conn()
    conn.execute("UPDATE content_posts SET body=%s WHERE id=%s AND business_id=%s",
...
def schedule_post(post_id, business_id, when_iso):
    ...
    conn = get_conn()
    conn.execute("UPDATE content_posts SET status='scheduled', scheduled_for=%s, decided_at=%s "
                 "WHERE id=%s AND business_id=%s",
...
def scheduled_post_times(business_id):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT scheduled_for FROM content_posts WHERE business_id=%s AND status='scheduled' "
...
def due_posts(now_iso_str=None):
    ...
    rows = conn.execute(
        ...
        "AND scheduled_for <= %s ORDER BY scheduled_for", (cutoff,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def content_stats(business_id):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT status, COUNT(*) AS n FROM content_posts WHERE business_id=%s "
...
def get_contact(contact_id, business_id=None):
    conn = get_conn()
    if business_id is None:
        row = conn.execute("SELECT * FROM contacts WHERE id=%s", (contact_id,)).fetchone()
    else:
        row = conn.execute("SELECT * FROM contacts WHERE id=%s AND business_id=%s",
...
def find_contact_by_phone(business_id, phone):
    conn = get_conn()
    row = conn.execute("SELECT * FROM contacts WHERE business_id=%s AND phone=%s",
...
def list_contacts(business_id, kind=None):
    conn = get_conn()
    if kind:
        rows = conn.execute("SELECT * FROM contacts WHERE business_id=%s AND kind=%s "
                            "ORDER BY created_at DESC", (business_id, kind)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM contacts WHERE business_id=%s "
...
def set_contact_suppressed(business_id, contact_id, suppressed=True):
    ...
    conn = get_conn()
    conn.execute("UPDATE contacts SET suppressed=%s WHERE id=%s AND business_id=%s",
...
def set_contact_consent(business_id, contact_id, channel, event, source=""):
    ...
    conn.execute("UPDATE contacts SET consent_status=%s WHERE id=%s AND business_id=%s",
...
def list_messages(business_id, limit=100):
    conn = get_conn()
    rows = conn.execute("SELECT * FROM messages WHERE business_id=%s "
                        "ORDER BY created_at DESC LIMIT %s", (business_id, limit)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_review(review_id, business_id=None):
    ...
    if business_id is None:
        ...
        row = conn.execute("SELECT * FROM reviews WHERE id=%s AND business_id=%s",
...
def list_reviews(business_id):
    conn = get_conn()
    rows = conn.execute("SELECT * FROM reviews WHERE business_id=%s ORDER BY created_at DESC",
                        (business_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def set_review_response(review_id, business_id, response, mark_responded=True):
    ...
    conn = get_conn()
    if mark_responded:
        conn.execute("UPDATE reviews SET response=%s, status='responded', responded_at=%s "
                     "WHERE id=%s AND business_id=%s",
                     (response, now_iso(), review_id, business_id))
    else:
        conn.execute("UPDATE reviews SET response=%s WHERE id=%s AND business_id=%s",
...
def add_conversion(business_id, channel, status="won", value=0.0, contact_id=None,
    ...
    cur = conn.execute(
        ...
        "label, origin, lead_id, ext_id, created_at, won_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
...
def conversion_exists(business_id, origin, ext_id):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT 1 FROM conversions WHERE business_id=%s AND origin=%s AND ext_id=%s LIMIT 1",
...
def roi_summary(business_id):
    ...
    spend_rows = conn.execute(
        ...
        "WHERE business_id=%s GROUP BY channel", (business_id,)).fetchall()
    conv_rows = conn.execute(
        "SELECT channel, status, COUNT(*) AS n, COALESCE(SUM(value),0) AS revenue "
        "FROM conversions WHERE business_id=%s GROUP BY channel, status",
        (business_id,)).fetchall()
    conn.close()
...
# ---- JobMagnet's engine: diagnostic signals + the Mandate ----
...
def get_signals(business_id):
    conn = get_conn()
    row = conn.execute("SELECT * FROM business_signals WHERE business_id=%s",
...
def save_mandate(business_id, plays):
    ...
    for p in plays:
        existing = conn.execute(
            "SELECT election FROM playbook_elections WHERE business_id=%s AND playbook=%s",
        ...
        conn.execute(
            ...
            "election, updated_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s) "
...
def has_mandate(business_id):
    conn = get_conn()
    n = conn.execute("SELECT COUNT(*) FROM playbook_elections WHERE business_id=%s",
...
def get_mandate(business_id):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM playbook_elections WHERE business_id=%s ORDER BY priority",
        (business_id,)).fetchall()
    conn.close()
...
def set_election(business_id, playbook, election):
    ...
    conn = get_conn()
    cur = conn.execute(
        "UPDATE playbook_elections SET election=%s, updated_at=%s "
        "WHERE business_id=%s AND playbook=%s",
...
def get_plan(business_id):
    conn = get_conn()
    row = conn.execute("SELECT plan FROM businesses WHERE id=%s", (business_id,)).fetchone()
    conn.close()
...
def set_plan(business_id, plan):
    if plan not in plans.PLANS:
        return False
    conn = get_conn()
    conn.execute("UPDATE businesses SET plan=%s WHERE id=%s", (plan, business_id))
    conn.commit()
    conn.close()
    return True
...
def get_auto_publish(business_id):
    ...
    conn = get_conn()
    row = conn.execute("SELECT auto_publish FROM businesses WHERE id=%s",
                       (business_id,)).fetchone()
    conn.close()
...
def set_auto_publish(business_id, on):
    ...
    conn = get_conn()
    conn.execute("UPDATE businesses SET auto_publish=%s WHERE id=%s",
...
def find_business_by_customer(customer_id):
    ...
    if not customer_id:
        return None
    conn = get_conn()
    row = conn.execute("SELECT * FROM businesses WHERE stripe_customer_id=%s",
...
def set_billing(business_id, customer_id=None, subscription_id=None, plan=None, status=None):
    ...
    if customer_id is not None:
        sets.append("stripe_customer_id=%s"); vals.append(customer_id)
    if subscription_id is not None:
        sets.append("stripe_subscription_id=%s"); vals.append(subscription_id)
    if status is not None:
        sets.append("plan_status=%s"); vals.append(status)
    if plan in plans.PLANS:
        sets.append("plan=%s"); vals.append(plan)
    if not sets:
        return
    conn = get_conn()
    conn.execute(f"UPDATE businesses SET {', '.join(sets)} WHERE id=%s", tuple(vals) + (business_id,))
    conn.commit()
    conn.close()
...
def _outbound_sms_since(business_id, start_iso):
    conn = get_conn()
    n = conn.execute(
        "SELECT COUNT(*) FROM messages WHERE business_id=%s AND channel='sms' "
        "AND direction='outbound' AND status IN ('sent','simulated') AND created_at>=%s",
        (business_id, start_iso)).fetchone()["count"]
    conn.close()
    return n
...
def start_or_get_convo(business_id, session_key):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT id, last_at FROM assistant_convos WHERE business_id=%s AND session_key=%s "
    ...
    if cid is None:
        cid = conn.execute(
            ...
            "VALUES (%s,%s,%s,%s) RETURNING id", (business_id, session_key or "", now, now)).fetchone()["id"]
    else:
        conn.execute("UPDATE assistant_convos SET last_at=%s WHERE id=%s", (now, cid))
    conn.commit()
    conn.close()
    return cid
...
def recent_user_turns(convo_id, business_id, limit=6):
    conn = get_conn()
    rows = conn.execute(
        "SELECT content FROM assistant_turns WHERE convo_id=%s AND business_id=%s AND role='user' "
        "ORDER BY id DESC LIMIT %s", (convo_id, business_id, limit)).fetchall()
    conn.close()
...
def add_flag(business_id, convo_id, turn_id, kind, detail=""):
    ...
    fid = conn.execute(
        ...
        "VALUES (%s,%s,%s,%s,%s,%s) RETURNING id",
        (business_id, convo_id, turn_id, kind, detail, now_iso())).fetchone()["id"]
    conn.commit()
    conn.close()
    return fid
...
def list_convos(business_id, limit=30):
    ...
    rows = conn.execute(
        ...
        "FROM assistant_convos c WHERE c.business_id=%s ORDER BY c.id DESC LIMIT %s",
...
def list_flags(business_id, resolved=0, limit=50):
    ...
    rows = conn.execute(
        ...
        "WHERE f.business_id=%s AND f.resolved=%s ORDER BY f.id DESC LIMIT %s",
...
def get_flag(business_id, flag_id):
    conn = get_conn()
    row = conn.execute("SELECT * FROM assistant_flags WHERE id=%s AND business_id=%s",
...
def resolve_flag(business_id, flag_id):
    conn = get_conn()
    conn.execute("UPDATE assistant_flags SET resolved=1 WHERE id=%s AND business_id=%s",
                 (flag_id, business_id))
    conn.commit()
    conn.close()
...
def add_learning(business_id, pattern, action, answer="", source_turn_id=None, confirmed=1):
    ...
    lid = conn.execute(
        ...
        "confirmed, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id",
        (business_id, (pattern or "").strip().lower(), action, answer, source_turn_id,
         1 if confirmed else 0, now_iso())).fetchone()["id"]
    conn.commit()
    conn.close()
    return lid
...
def list_learnings(business_id, confirmed_only=True):
    conn = get_conn()
    q = "SELECT * FROM assistant_learnings WHERE business_id=%s"
    if confirmed_only:
        q += " AND confirmed=1"
...
def bump_learning(business_id, learning_id):
    conn = get_conn()
    conn.execute("UPDATE assistant_learnings SET uses=uses+1 WHERE id=%s AND business_id=%s",
...
def memory_digest(business_id, since_iso):
    ...
    conn = get_conn()
    flags = conn.execute(
        "SELECT kind, COUNT(*) AS n FROM assistant_flags WHERE business_id=%s AND created_at>=%s "
        "GROUP BY kind", (business_id, since_iso)).fetchall()
    learns = conn.execute(
        "SELECT COUNT(*) FROM assistant_learnings WHERE business_id=%s AND created_at>=%s",
        (business_id, since_iso)).fetchone()["count"]
    convs = conn.execute(
        "SELECT COUNT(*) FROM assistant_convos WHERE business_id=%s "
        "AND COALESCE(last_at, started_at)>=%s", (business_id, since_iso)).fetchone()["count"]
    conn.close()
...
def unmet_flag_contents(business_id):
    ...
    rows = conn.execute(
        ...
        "WHERE f.business_id=%s AND f.resolved=0 AND f.kind IN ('capability_gap','unhelpful') "
...
def coach_candidates(business_id):
    """(message, route) for every open capability-gap whose flag recorded the page JobMagnet
    ...
    rows = conn.execute(
        ...
        "WHERE f.business_id=%s AND f.resolved=0 AND f.kind='capability_gap' "
...
def convo_user_turn_count(business_id, convo_id):
    conn = get_conn()
    n = conn.execute("SELECT COUNT(*) FROM assistant_turns WHERE business_id=%s AND convo_id=%s "
                     "AND role='user'", (business_id, convo_id)).fetchone()["count"]
    conn.close()
    return n


def has_coach_offer(business_id, convo_id):
    """True once JobMagnet has already made a teaching offer in this conversation (so he asks
    at most once per chat)."""
    conn = get_conn()
    n = conn.execute("SELECT COUNT(*) FROM assistant_flags WHERE business_id=%s AND convo_id=%s "
                     "AND kind='coach_offered'", (business_id, convo_id)).fetchone()["count"]
    conn.close()
    return n > 0
...
def mark_coach_offered(business_id, convo_id):
    ...
    conn.execute(
        ...
        "created_at, resolved) VALUES (%s,%s,%s,%s,%s,%s,1)",
...
def log_autopilot_run(business_id, posts, msgs, capped, sms_mode, origin="manual"):
    """Record one autopilot run so the owner can see what JobMagnet did on his own."""
    conn = get_conn()
    rid = conn.execute(
...
def list_autopilot_runs(business_id, limit=20):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM autopilot_runs WHERE business_id=%s ORDER BY id DESC LIMIT %s",
        (business_id, limit)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def last_autopilot_run(business_id):
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM autopilot_runs WHERE business_id=%s ORDER BY id DESC LIMIT 1",
        (business_id,)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def set_connection(business_id, provider, creds):
    ...
    conn.execute(
        ...
        "VALUES (%s,%s,%s,%s,%s) ON CONFLICT(business_id, provider) DO UPDATE SET "
...
def get_connection(business_id, provider):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT credentials, status FROM connections WHERE business_id=%s AND provider=%s",
...
def disconnect(business_id, provider):
    conn = get_conn()
    conn.execute("UPDATE connections SET status='disconnected' WHERE business_id=%s AND provider=%s",
...
def connection_status(business_id):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT provider, status FROM connections WHERE business_id=%s", (business_id,)).fetchall()
    conn.close()
...
def get_getfound_done(business_id):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT item FROM getfound_checklist WHERE business_id=%s AND done=1",
        (business_id,)).fetchall()
    conn.close()
...
def set_getfound_item(business_id, item, done):
    ...
    conn.execute(
        ...
        "VALUES (%s,%s,%s,%s) ON CONFLICT(business_id, item) DO UPDATE SET "
...
# ---- LSA Concierge checklist (shares the checklist store; keys are lsa_-prefixed) ----

def get_lsa_done(business_id):
    """The set of completed LSA checklist item keys for a tenant."""
    conn = get_conn()
    rows = conn.execute(
        "SELECT item FROM getfound_checklist WHERE business_id=%s AND done=1",
        (business_id,)).fetchall()
    conn.close()
    return {r["item"] for r in rows if r["item"] in lsa.CHECKLIST_KEYS}


def set_lsa_item(business_id, item, done):
    """Mark one LSA checklist item done/undone. Validates against lsa.CHECKLIST_KEYS."""
    if item not in lsa.CHECKLIST_KEYS:
        return False
    conn = get_conn()
    conn.execute(
        "INSERT INTO getfound_checklist (business_id, item, done, updated_at) "
...
# ---- Neighbor Mail: radius campaigns per completed jobsite ----

def add_mail_campaign(business_id, camp, contact_id=None):
    """Persist a draft campaign dict from radiusmail.campaign_from_job(). Returns id."""
    conn = get_conn()
    row = conn.execute(
        "INSERT INTO mail_campaigns (business_id, contact_id, job_address, service, "
        "pieces, letter_body, hanger_body, status, mode, created_at) "
        "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
        (business_id, contact_id, camp["job_address"], camp.get("service"),
         camp.get("pieces", 50), camp.get("letter_body"), camp.get("hanger_body"),
         camp.get("status", "draft"), camp.get("mode", "assisted"),
         now_iso())).fetchone()
    conn.commit()
    conn.close()
    return row["id"]
...
def list_mail_campaigns(business_id):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM mail_campaigns WHERE business_id=%s ORDER BY created_at DESC, id DESC",
        (business_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_mail_campaign(campaign_id, business_id):
    conn = get_conn()
    row = conn.execute(
        "SELECT * FROM mail_campaigns WHERE id=%s AND business_id=%s",
        (campaign_id, business_id)).fetchone()
    conn.close()
    return dict(row) if row else None


def set_mail_campaign_status(business_id, campaign_id, status):
    """Advance a campaign through draft -> approved -> printed. Validates the status."""
    if status not in radiusmail.STATUSES:
        return False
    conn = get_conn()
    conn.execute(
        "UPDATE mail_campaigns SET status=%s, printed_at=CASE WHEN %s='printed' "
        "THEN %s ELSE printed_at END WHERE id=%s AND business_id=%s",
        (status, status, now_iso(), campaign_id, business_id))
    conn.commit()
    conn.close()
    return True


def mail_campaign_contact_ids(business_id):
    """Contacts that already anchor a campaign, so autopilot never drafts twice
    for the same job."""
    conn = get_conn()
    rows = conn.execute(
        "SELECT DISTINCT contact_id FROM mail_campaigns WHERE business_id=%s "
        "AND contact_id IS NOT NULL", (business_id,)).fetchall()
    conn.close()
    return {r["contact_id"] for r in rows}


def log_llm_usage(model, input_tokens, output_tokens, cost_usd,
                  *, business_id=None, tier=None, provider=None):
    """Append one LLM call to the cost ledger. Swallows nothing here (brain.py wraps the
    call and never lets a logging failure break the return contract). business_id=None
    records a platform-level (non-tenant) call."""
    conn = get_conn()
    conn.execute(
        "INSERT INTO llm_usage (business_id, tier, provider, model, input_tokens, "
        "output_tokens, cost_usd, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
        (business_id, tier, provider, model, int(input_tokens or 0),
         int(output_tokens or 0), float(cost_usd or 0.0), now_iso()))
    conn.commit()
    conn.close()


def llm_cost_today(business_id=None):
    """Total LLM spend (USD) logged since UTC midnight today. Platform-wide when
    business_id is None, else scoped to that tenant. Returns 0.0 if no rows. Raises on a
    genuine DB error (brain.py treats that as fail-open, not a cap hit)."""
    ...
    if business_id is None:
        row = conn.execute(
            "SELECT COALESCE(SUM(cost_usd), 0.0) AS c FROM llm_usage WHERE created_at >= %s",
            (start,)).fetchone()
    else:
        row = conn.execute(
            "SELECT COALESCE(SUM(cost_usd), 0.0) AS c FROM llm_usage "
            "WHERE created_at >= %s AND business_id = %s", (start, business_id)).fetchone()
    conn.close()
    return float(row["c"]) if row else 0.0


def set_contact_partner_type(business_id, contact_id, partner_type):
    conn = get_conn()
    conn.execute("UPDATE contacts SET partner_type=%s WHERE id=%s AND business_id=%s",
                 ((partner_type or "").strip() or None, contact_id, business_id))
    conn.commit()
    conn.close()


def set_contact_address(business_id, contact_id, address):
    conn = get_conn()
    conn.execute("UPDATE contacts SET address=%s WHERE id=%s AND business_id=%s",
                 ((address or "").strip() or None, contact_id, business_id))
    conn.commit()
    conn.close()
...
def list_leads(business_id):
    conn = get_conn()
    rows = conn.execute("SELECT * FROM leads WHERE business_id=%s ORDER BY created_at DESC",
                        (business_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_lead(lead_id, business_id=None):
    ...
    if business_id is None:
        ...
        row = conn.execute("SELECT * FROM leads WHERE id=%s AND business_id=%s",
...
def mark_lead_responded(business_id, lead_id):
    ...
    conn = get_conn()
    conn.execute(
        "UPDATE leads SET first_response_at=COALESCE(first_response_at, %s), "
        "status=CASE WHEN status='new' THEN 'responded' ELSE status END "
        "WHERE id=%s AND business_id=%s", (now_iso(), lead_id, business_id))
    conn.commit()
    conn.close()
...
def set_lead_status(business_id, lead_id, status, value=None):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT channel, status, name, value FROM leads WHERE id=%s AND business_id=%s",
        (lead_id, business_id)).fetchone()
    if not row:
        conn.close()
        return False
    if value is not None:
        conn.execute("UPDATE leads SET status=%s, value=%s WHERE id=%s AND business_id=%s",
                     (status, max(0.0, float(value or 0)), lead_id, business_id))
    else:
        conn.execute("UPDATE leads SET status=%s WHERE id=%s AND business_id=%s",
...
def _sync_lead_conversion(business_id, lead_id, channel, status, value, label):
    ...
    conn = get_conn()
    existing = conn.execute(
        "SELECT id FROM conversions WHERE business_id=%s AND lead_id=%s AND origin='speed_to_lead'",
        (business_id, lead_id)).fetchone()
    if existing:
        won_at = now_iso() if status == "won" else None
        conn.execute(
            "UPDATE conversions SET status=%s, value=%s, channel=%s, "
            "won_at=COALESCE(%s, won_at) WHERE id=%s",
...
def set_contact_job(business_id, contact_id, last_job_at, last_service):
    conn = get_conn()
    conn.execute("UPDATE contacts SET last_job_at=%s, last_service=%s "
                 "WHERE id=%s AND business_id=%s",
...
def last_post_at(business_id, platform):
    ...
    conn = get_conn()
    row = conn.execute(
        "SELECT MAX(created_at) AS last FROM content_posts WHERE business_id=%s AND platform=%s",
...
def contacted_ids(business_id, purpose):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT DISTINCT contact_id FROM messages WHERE business_id=%s AND purpose=%s "
...
def review_requested_to_phone(business_id, phone):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT to_addr FROM messages WHERE business_id=%s AND purpose='review_request' "
...
def review_stats(business_id):
    ...
    conn = get_conn()
    rows = conn.execute("SELECT rating, status, created_at FROM reviews WHERE business_id=%s",
                        (business_id,)).fetchall()
    requested = conn.execute(
        "SELECT COUNT(*) FROM messages WHERE business_id=%s AND purpose='review_request'",
        (business_id,)).fetchone()["count"]
    conn.close()
    total = len(rows)
