...
- Signed-in scope: **add Mason's conversational home + polish every page.**
...
- `/dashboard` must keep the content approval queue strings: "Awaiting your
  review", "Approve", "Published" (tests assert them). → Mason's home is built
...
2. **Mason home** — redesign `/dashboard` into "Mason's Today": briefing hero +
...
4. **Signed-in polish** — elevate Game Plan, Reviews, Contacts, Results,
   Settings, Compose, Get Found, Plan with Mason voice + stronger states.
...
- Public site sells the price point (clear value, Mason, proof, pricing, CTA).
- Mason home is the post-login front door; every signed-in page reads premium.
